...
int runPythonApp() {
    ...
    @autoreleasepool {
        ...
        python_tag = @"3.10";
        python_home = [NSString stringWithFormat:@"%@", resourcePath, nil];
        ...
        path = [NSString stringWithFormat:@"%@/Frameworks/Python.framework/lib/python%@", python_home, python_tag, nil];
        NSLog(@"- %@", path);
        ...
        path = [NSString stringWithFormat:@"%@/Frameworks/Python.framework/lib/python%@/lib-dynload", python_home, python_tag, nil];
        NSLog(@"- %@", path);
        wtmp_str = Py_DecodeLocale([path UTF8String], NULL);
        status = PyWideStringList_Append(&config.module_search_paths, wtmp_str);
        if (PyStatus_Exception(status)) {
